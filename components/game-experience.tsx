...
import { ArrowLeft, ArrowRight, Copy, RefreshCcw, Users } from "lucide-react";
...
function GameCanvas({ game, mode, bots, room, playerId, onRoomUpdate }: {
  ...
  const inputRef = useRef<GameInput>({ x: 0, y: 0, sprint: false, kickSeq: 0, aimX: 0, aimY: 0, power: 0, spin: false });
  ...
  useEffect(() => {
    ...
    const onDown = (event: KeyboardEvent) => {
      if (["KeyW", "KeyA", "KeyS", "KeyD", "KeyQ", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "ShiftLeft", "ShiftRight"].includes(event.code)) {
    ...
    const onBlur = () => { keysRef.current.clear(); inputRef.current.x = 0; inputRef.current.y = 0; };
  ...
  useEffect(() => {
    ...
    const run = (now: number) => {
      ...
      inputRef.current.spin = keys.has("KeyQ");
  ...
  return <div className="game-board-wrap">
    ...
    <div className="game-canvas-shell">
      <canvas ref={canvasRef} tabIndex={0} className="game-canvas" aria-label={`${labels[game]}: use WASD para mover${game === "haxball" ? " e o mouse para chutar" : ""}`} onPointerDown={(event) => { pointerDownAt.current = performance.now(); event.currentTarget.focus(); }} onPointerUp={onPointerUp} onContextMenu={(event) => event.preventDefault()} />
      {mode === "online" && room && !room.players.some((p) => p.team === "orange") && <div className="game-waiting">Aguardando adversário na sala {room.id}</div>}
      {hud.winner && <div className="game-waiting">Equipe {hud.winner === "blue" ? "branca" : "laranja"} venceu.</div>}
    </div>
    <div className="game-board-footer"><span>WASD ou setas: mover {game === "haxball" ? " · Clique e segure: carregar chute · Shift: correr · Q: curva" : " · Empurre o disco para marcar"}</span>{(mode === "bot" || isHost) && <button type="button" onClick={reset}><RefreshCcw size={15} /> Reiniciar</button>}</div>
...
export function GameExperience({ game }: { game: GameId }) {
  ...
  return <main className="game-experience shell">
    ...
    {(mode === "bot" || room) && <GameCanvas game={game} mode={mode} bots={bots} room={room} playerId={playerId} onRoomUpdate={onRoomUpdate} />}
